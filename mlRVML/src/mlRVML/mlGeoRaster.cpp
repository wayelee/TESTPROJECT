...
bool CmlGeoRaster::SaveToGeoFile( vector<double> &vecZVal )
{
    ...
    if( (vecZVal.size() != (UINT) GetHeight()*GetWidth()) ||(vecZVal.size() == 0))
    {
        return false;
    }
...
bool CmlGeoRaster::SaveToGeoFile(int nBand, int nXOffSet, int nYOffSet, CmlRasterBlock* pBlock )
{
    ...
    return this->SaveBlockToFile( nBand, nXOffSet, nYOffSet, pBlock, 1 );

}
bool CmlGeoRaster::GetGeoInfo( const SCHAR* strGeoFile, Pt2d &ptOrig, DOUBLE &dXRes, DOUBLE &dYRes, UINT &nW, UINT &nH, DOUBLE &dNoDataVal )
{
    if( false == this->LoadGeoFile( strGeoFile ) )
    {
        return false;
    }
    ptOrig = m_PtOrigin;
    dXRes = m_dXResolution;
    dYRes = m_dYResolution;

    nW = GetWidth();
    nH = GetHeight();

    dNoDataVal = GetNoDataVal();
    return true;
}
bool CmlGeoRaster::SetGeoInfo( const SCHAR* strGeoFile, Pt2d ptOrig, DOUBLE dXRes, DOUBLE dYRes, DOUBLE dNoDataVal )
{
    GDALAllRegister();
    GDALDataset *pDataset = NULL;
    pDataset = (GDALDataset*)GDALOpen( strGeoFile, GA_Update );
    if( pDataset == NULL )
    {
        return false;
    }
    ...
    pDataset->SetGeoTransform( dPara );
    ...
    GDALSetRasterNoDataValue( pBand, dNoDataVal );
    GDALClose( pDataset );
    return true;
}
