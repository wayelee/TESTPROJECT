...
#if ( defined WIN32 || defined _WIN32 || defined WIN64 || defined _WIN64 ) && defined MLAPI_EXPORTS
...
//    UINT nRH;//!>右相机影像高
//    UINT nRW;//!>右相机影像宽
//    UINT nImgLIndex;//!>左影像索引号
//    UINT nImgRIndex;//!>右影像索引号

//    string strLName;//!>左影像名
//    string strLImgPath;//!>左影像路径
//    string strRName;//!>右影像名
//    string strRImgPath;//!>右影像路径
//    InOriPara inOriL;//!>左影像内方位
//    InOriPara inOriR;//!>左影像内方位
//    ExOriPara exOriL;//!>右影像外方位
//    ExOriPara exOriR;//!>右影像外方位
//    SINT nImgLevel; //!>表示影像类型，1为原始影像，2为核线影像
...
//        nRollID = -1;
//        nImgID = -1;
//        strLImgPath = strRImgPath = "";
//        nImgLevel = -1;
//        nImgLIndex = nImgRIndex = 0;
//    }
...
typedef struct  tagSatOptions
{
    ...
    DOUBLE dSigma;//!<Ransac算法限差
    UINT nMinItera;//!<Ransac算法最小迭代次数
    UINT nStep;//!<密集匹配步长
...
typedef struct  tagLocalByMatchOptions
{
    ...
    RANSACPara stuRANSACPara;//!<RANSAC参数
} LocalByMatchOpts;
...
{
    DOUBLE dXMin;//!<X方向区域最小时差
    DOUBLE dYMin;//!<Y方向区域最大时差
    DOUBLE dXMax;//!<X方向区域最小时差
    DOUBLE dYMax;//!<Y方向区域最大时差
...
typedef struct tagLocalByLanderOpts
{
    ...
    RANSACPara stuRANSACPara;//!<RANSAC粗差剔除参数
}
LocalByLanderOpts;
