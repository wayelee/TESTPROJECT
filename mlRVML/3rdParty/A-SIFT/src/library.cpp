...
float max(float *u,int *pos, int size)
{
    float max=u[0];
    if (pos) *pos=0;
    for(int i=1; i<size; i++)  if(u[i]>max)
        {
            max=u[i];
            if (pos) *pos=i;
        }
...
float min(float *u,int *pos,int size)
{
    float min=u[0];
    if (pos) *pos=0;
    for(int i=1; i<size; i++)	if(u[i]<min)
        {
            min=u[i];
            if (pos) *pos=i;
        }
...
{
    for(int i=0; i<size; i++)  u[i]=MAX(u[i],v[i]);
}

void max_u_k(float *u, float k, int size)
{
    for(int i=0; i<size; i++)  u[i]=MAX(u[i],k);
}

void min_u_v(float *u, float *v,int size)
{
    for(int i=0; i<size; i++)  u[i]=MIN(u[i],v[i]);
}

void min_u_k(float *u,float k,int size)
{
    int i=0;
    for(i=0; i<size; i++)  u[i]=MIN(u[i],k);
}

void abs(float *u,float *v,int size)
{
    for(int i=0; i<size ; i++)  v[i] = fabsf(u[i]);
}

void copy(float *u,float *v,int size)
{
    for(int i=0; i<size ; i++)  v[i]=u[i];
}

void clear(float *u,float value,int size)
{
    for(int i=0; i<size; i++) u[i]=value;
}
...
void multiple(float *u,float multiplier,int size)
{
    for(int i=0; i<size; i++)  u[i]=multiplier*u[i];
}
...
float * histo(float* input, float *iminim, float *imaxim, int *n, float *s, int size, char flag)
{
    ...
    if (flag != 's' && flag != 'n')
    {
        printf("Warning (histo): Please select s or n as flag\n");
        return NULL;
    }
