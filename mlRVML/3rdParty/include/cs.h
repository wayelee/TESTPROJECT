...
#define CSPARSE_CDECL __cdecl
#define CSPARSE_STDCALL __stdcall
#else
#define CSPARSE_CDECL
#define CSPARSE_STDCALL
#endif

#ifndef CSPARSE_EXTERN_C
#ifdef __cplusplus
#define CSPARSE_EXTERN_C extern "C"
#define CSPARSE_DEFAULT(val) = val
#else
#define CSPARSE_EXTERN_C
#define CSPARSE_DEFAULT(val)
#endif
#endif

#if ( defined WIN32 || defined _WIN32 || defined WIN64 || defined _WIN64 ) && (defined CSPARSE_EXPORTS)
#define CSPARSEDLL_EXPORTS __declspec(dllexport)
#else
#define CSPARSEDLL_EXPORTS
#endif

#ifndef CSPARSE_API
#define CSPARSE_API(rettype) CSPARSE_EXTERN_C CSPARSEDLL_EXPORTS rettype CSPARSE_CDECL
#endif
//------------------------------------
...
CSPARSE_API(cs*) cs_add (const cs *A, const cs *B, double alpha, double beta) ;
CSPARSE_API(csi) cs_cholsol (csi order, const cs *A, double *b) ;
CSPARSE_API(cs*) cs_compress (const cs *T) ;
CSPARSE_API(csi) cs_dupl (cs *A) ;
CSPARSE_API(csi) cs_entry (cs *T, csi i, csi j, double x) ;
CSPARSE_API(csi) cs_gaxpy (const cs *A, const double *x, double *y) ;
CSPARSE_API(cs*) cs_load (FILE *f) ;
CSPARSE_API(csi) cs_lusol (csi order, const cs *A, double *b, double tol) ;
CSPARSE_API(cs*) cs_multiply (const cs *A, const cs *B) ;
CSPARSE_API(double) cs_norm (const cs *A) ;
CSPARSE_API(csi) cs_print (const cs *A, csi brief) ;
CSPARSE_API(csi) cs_qrsol (csi order, const cs *A, double *b) ;
CSPARSE_API(cs*) cs_transpose (const cs *A, csi values) ;
/* utilities */
CSPARSE_API(void*) cs_calloc (csi n, size_t size) ;
CSPARSE_API(void*) cs_free (void *p) ;
CSPARSE_API(void*) cs_realloc (void *p, csi n, size_t size, csi *ok) ;
CSPARSE_API(cs*) cs_spalloc (csi m, csi n, csi nzmax, csi values, csi triplet) ;
CSPARSE_API(cs*) cs_spfree (cs *A) ;
CSPARSE_API(csi) cs_sprealloc (cs *A, csi nzmax) ;
CSPARSE_API(void*) cs_malloc (csi n, size_t size) ;
