...
{
    CmlPhgProc clsPhg;
	ExOriPara exOriBase, exOriTmp1, exOriTmp2, exOriPreTmp1, exOriPre;
	exOriTmp1.ori.omg = Deg2Rad( 180 );
	exOriTmp2.ori.kap = Deg2Rad( -90 );

	clsPhg.ExOriTrans( &exOriBase, &exOriTmp1, &exOriPreTmp1 );
	clsPhg.ExOriTrans( &exOriPreTmp1, &exOriTmp2, &exOriPre );

    //先由本体系转至东北天坐标系，也可不需要
    ExOriPara oriBase, oriExp, oriBase1;
	...
	clsPhg.ExOriTrans( &exOriPre, &oriBase, &oriBase1 );
    //桅杆相对本体矩阵
    calcOriByMat( oriBase1, matMastExp2Body, oriExp );
