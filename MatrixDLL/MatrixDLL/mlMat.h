...
public:
    ...
    */
    ...
    CmlTemplateMat<T>& operator=( const CmlTemplateMat<T>& tmp); //!<复制拷贝函数
...
template <typename T>
CmlTemplateMat<T>& CmlTemplateMat<T>::operator=( const CmlTemplateMat<T>& tmp )//!<复制拷贝函数
{
    if( true != m_bIsNULL )
    {
        this->DestoryAll();
...
//AT*A
MLAPI(bool) mlMatMulTrans( CmlMat* pMatA, CmlMat* pMatRes );
//(AT*A)-1
MLAPI(bool) mlMatMulTransInv( CmlMat* pMatA, CmlMat* pMatRes );
/**
* @fn CmlMat2CvMat
* @date 2011.11.02
* @author 万文辉 whwan@irsa.ac.cn
