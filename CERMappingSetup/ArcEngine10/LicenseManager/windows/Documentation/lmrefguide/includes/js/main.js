	...
	// page title are appended to this base title
	var standardPageTitle = "ESRI Help";
	...
	var TOC = {
		...
		init : function(){
			...
			lazyTree.successHandler = function(data, textStatus, treeNode){
				...
				if(isRootRequest){
					...
					lazyTree.createBranch(treeData[data.rootElement][lazyTree.getTreeOption('childrenProperty')], treeNode);
			...
			lazyTree.createBranch = function(children, treeNode){
				...
				if(typeof children != UNDEFINED) 
					inlineMaps = $.grep(children, function(elem, index){ return (elem.substr(0,4).toLowerCase() == "map_") });
				...
				var nodeID =  treeNode.data('nodeData')[lazyTree.getTreeOption('idProperty')];
			...
			lazyTree.nodeClickHandler = function(evt){
				...
				if(lazyTree.isBranch(treeNode)){
					if(lazyTree.branchHasChildren(treeNode)){
						...
						if(typeof treeNode.data('nodeData')[lazyTree.getTreeOption('mapProperty')] != UNDEFINED){	// is map
							lazyTree.loadBranches(treeNode);
						}else{
							var nodeID = treeNode.data('nodeData')[lazyTree.getTreeOption('idProperty')];
							var nodeChildren = tocMap.tree[nodeID][lazyTree.getTreeOption('childrenProperty')];
			...
			lazyTree.getAjaxURL = function(treeNode){
				if(typeof treeNode.data('nodeData')[lazyTree.getTreeOption('mapProperty')] != UNDEFINED){
					return treeNode.data('nodeData')[lazyTree.getTreeOption('mapProperty')] + '/' + lazyTree.getTreeOption('ajaxURL');
				}else{
			...
			lazyTree.createTreeNode = function(data){
				...
				var nodeType = (typeof data[lazyTree.getTreeOption('childrenProperty')] != UNDEFINED ||
								typeof data[lazyTree.getTreeOption('mapProperty')] != UNDEFINED) 
				...
				var newNode = 	$('<li class="' + nodeType + '" rel="">' + '<a href="' + 
								HelpDoc.getDocPath( data[lazyTree.getTreeOption('idProperty')] ) + '" rel="' +
								data[lazyTree.getTreeOption('idProperty')] + '" title="' + 
								data[lazyTree.getTreeOption('titleProperty')] + '">' +
								data[lazyTree.getTreeOption('titleProperty')] + '</a></li>');
			...
			lazyTree.loadLeaf = function(leaf){
				var leafValue = leaf.data('nodeData')[lazyTree.getTreeOption('idProperty')];
		...
		getPathFromRoot : function(id){
			...
			while(id.length && loopCount < 100){
				...
				if(tocMap.tree.hasOwnProperty(id) && tocMap.tree[id].hasOwnProperty(lazyTree.getTreeOption('parentProperty'))){
					// get the parent id
					var parentID = tocMap.tree[id][lazyTree.getTreeOption('parentProperty')];
					if(parentID.length == 0){
						...
						if(parentMap.length > 0) {
							...
							if (parentMap.length > 0) {
								...
								parentMap = TOC.fixInlineMapParent(tocMap.tree[id][lazyTree.getTreeOption('childrenProperty')][0]);
								if (parentMap.length > 0) {
									parentID = parentMap;
								}
							}
					...
					if(parentID.length && tocMap.tree[parentID][lazyTree.getTreeOption('parentProperty')]){
					...
					$.get(	mapValue.substr(4) + '/' + lazyTree.getTreeOption('ajaxURL'),
							...
							function(data,textStatus){
								if(textStatus == 'success'){
									var newNodes = "";
									var rootChildren = data.tree[data.rootElement][lazyTree.getTreeOption('childrenProperty')];
		...
		getMapParent : function(id){
			...
			$.each(tocMap.tree, function(){
				if(	this[lazyTree.getTreeOption('mapProperty')] == id){
					// set parentID and return false to break out of the $.each() loop
					parentID = this[lazyTree.getTreeOption('idProperty')];
					return false;
				}
			});
			return parentID;
		...
		fixInlineMapParent : function(id){
			...
			// loop through each item in the tocMap to find the element that matches
			$.each(tocMap.tree, function(){
				if(this.hasOwnProperty(lazyTree.getTreeOption('childrenProperty')) && 
						 $.inArray(id, this[lazyTree.getTreeOption('childrenProperty')]) > -1){
					// set parentID and return false to break out of the $.each() loop
					parentID = this[lazyTree.getTreeOption('idProperty')];
					return false;
				}
		...
		getInlineMapParent : function(id){
			...
			$.each(tocMap.tree, function(){
				if(this.hasOwnProperty(lazyTree.getTreeOption('childrenProperty')) && 
						 $.inArray('map_'+id, this[lazyTree.getTreeOption('childrenProperty')]) > -1){
					// set parentID and return false to break out of the $.each() loop
					parentID = this[lazyTree.getTreeOption('idProperty')];
					return false;
				}
		...
		openToNode : function(nodeID, endNode){
			...
			TOC.recursionCount++;
			if(TOC.recursionCount > 50) return false;
			...
			if(tocMap.tree.hasOwnProperty(nodeID)){
				...
				if(parentID == "") parentID = TOC.getInlineMapParent(mapID);				
				if(parentID != ""){
		...
		openNewBranch : function(treeNode){
			...
			var children = treeNode.data('nodeData')[lazyTree.getTreeOption('childrenProperty')];
			...
			if(children){
				...
				lazyTree.createBranch(children, treeNode);
			}else if(typeof treeNode.data('nodeData')[lazyTree.getTreeOption('mapProperty')] != UNDEFINED){
	...
	var HelpDoc = {
		...
		getDocHandler : function(result){
			...
			if(lazyTree.getSelectedNode().length && lazyTree.getSelectedNode().data('nodeData')){
				HelpDoc.setMetaData(lazyTree.getSelectedNode().data('nodeData'));
			}else{
				var pageID = HashManager.getHash().token;
				if(tocMap.tree.hasOwnProperty(pageID)){
					HelpDoc.setMetaData(tocMap.tree[pageID]);
				}
			}
		...
		errorHandler : function(XMLHttpRequest, textStatus, errorThrown){
			if(XMLHttpRequest.status == 404 || errorThrown.name == 'NS_ERROR_DOM_BAD_URI'){
				$("#docWrapper").html("<h1>Document not found!</h1>").fadeIn("normal");
				if(typeof initNavBar != UNDEFINED) {
					initNavBar();
				}
			}else{
				$("#docWrapper").html("<h1>An error has occurred, please try again!</h1>").fadeIn("normal");
				if(typeof initNavBar != UNDEFINED) {
					initNavBar();
				}
			}
		...
		setMetaData : function(nodeData){
			var id = nodeData[lazyTree.getTreeOption('idProperty')];
			var title = nodeData[lazyTree.getTreeOption('titleProperty')];
			var slug = title.replace(/\W/g, "_").replace (/_{2,}/g, "_").replace (/^_|_$/g,"");
			var slug = slug.slice (0, 256); 
		...
		setPageTitle : function(title){
			$.address.title(title + " : " + standardPageTitle);
		},
		...
		scrollToAnchor : function(anchorName){
			...
			if($anchorElem.length == 0) $anchorElem = $('#'+anchorName);
		...
		getDocPath : function(path, ext){
			if(typeof ext == UNDEFINED) ext = '.htm';
	...
	*/
	var HashManager = {
		...
		addressChangeHandler : function(event){
			...
			if(!requestedID.length) requestedID = tocMap.startNode;
		...
		setHash : function(page, slug, anchr){
			...
			if(HashManager.getHash().token == page) return;
			var hashValue = "/";
			if(slug.length > 0) hashValue += slug + '/';
			hashValue += page + '/';
			if(anchr.length > 0) hashValue += anchr + '/';
	...
	*/
	var ClickHijacker = {
		...
		attachInterceptions : function(){
			...
			$("#docWrapper a").live("click", ClickHijacker.interceptClick);
		...
		interceptClick : function(event){
			...
			else if($(event.target).attr('esrisubtype') == 'extrel') return;
		...
		parseURL : function(url){
			...
			if(url.match(/^((((http|https|ftp):\/\/)+)|(www))/)) return true;
