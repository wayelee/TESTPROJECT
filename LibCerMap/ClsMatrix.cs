...
namespace LibCerMap
{
    public class Matrix
    {
        ...
        //矩阵相加实现
        public Matrix Add(Matrix lm, Matrix rm)
        {
            //行出错
            if (lm.row != rm.row)
            {
        ...
        //矩阵相乘实现
        public Matrix Mutiply(Matrix m1, Matrix m2)
        {
            double temp = 0;
            Matrix ret;
            if (m1.col != m2.row)
        ...
        public void coord_Trans(double[] Array, double[] rotateMat, double[] tranVec, double[] result)
        {
