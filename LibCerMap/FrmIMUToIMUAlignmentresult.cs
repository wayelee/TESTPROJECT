    ...
    {         
        ...
        private DataRow GetHitDataRow(Point FirstPoint, DevExpress.XtraCharts.Series ss, Axis2D xAxis, Axis2D yAxis)
        {
            ...
            if (ss.Points.Any(x => PointHitChartData(FirstPoint, x as SeriesPoint ,xAxis, yAxis)))
            {
               ...
               if (ss.Equals(chartControl3.Series[2]))
               {
                   return sourcetable.Rows[(int)pt.Tag];
               }
            }
        ...
        private void ChartControl3_MouseDragCompleted(object sender, Point FirstPoint, Point SecondPoint)
        {
            ...
            if (chartZoomHelper.CurrentTool == ChartControlToolTypeEnum.ManualMatch)
            {
                 ...
                 if (GetHitDataRow(SecondPoint, chartControl3.Series[0], diagram.AxisX, diagram.SecondaryAxesY[0]) != null &&
                      GetHitDataRow(FirstPoint, chartControl3.Series[2], diagram.AxisX, diagram.AxisY) != null)
                 {
                     DataRow centerlinerow = GetHitDataRow(SecondPoint, chartControl3.Series[0], diagram.AxisX, diagram.SecondaryAxesY[0]);
                     DataRow IMURow = GetHitDataRow(FirstPoint, chartControl3.Series[2], diagram.AxisX, diagram.AxisY);
                     if (MatchedRowList.Keys.Contains(IMURow) == false && MatchedRowList.Values.Contains(centerlinerow) == false)
                     {
                         if (ValidManuallySelectMatchRow(IMURow, centerlinerow))
                         {
        ...
        private void ExportInsideToCenterline()
        {
            ...
            string BookMarkName = "内检测统计表格";
            DataTable dt = CreateInsidPointStatisticsTable();
            ExportDatatableToWordBookMark(dt, BookMarkName, doc);

            dt = CreateCenterlinePointStatisticsTable();
            BookMarkName = "管道中线数据概况";
            ExportDatatableToWordBookMark(dt, BookMarkName, doc);


            BookMarkName = "管节统计长度表";
            ...
            BookMarkName = "大于12点3m的管节列表";
            dt = CreateExceedPipesegmentTable();
            ExportDatatableToWordBookMark(dt, BookMarkName, doc);
        ...
        private DataTable CreateCenterlinePointStatisticsTable()
        {
            ...
            dr["总里程"] = Math.Round( CenterlinePointTable.AsEnumerable().Max(x => Convert.ToDouble(x[EvConfig.CenterlineMeasureField])) , 2);
        ...
        private DataTable CreateInsidPointStatisticsTable()
        {
            ...
            DataRow dr = dt.NewRow();
            dr[0] = "检测长度";
            dr[1] =Math.Round (stable.AsEnumerable().Max(x => Convert.ToDouble(x[EvConfig.IMUMoveDistanceField]))
                - stable.AsEnumerable().Min(x => Convert.ToDouble(x[EvConfig.IMUMoveDistanceField])), 2).ToString() + "m";
            dt.Rows.Add(dr);
            dr = dt.NewRow();
            dr[0] = "阀门数量";
            ...
            dr[0] = "钢管数量";
            dr[1] = ((from DataRow r in stable.Rows
                      where r[EvConfig.IMUPointTypeField] != DBNull.Value && r[EvConfig.IMUPointTypeField].ToString().Contains("钢管")
                     select r).Count() ).ToString() + "根"; 
            dt.Rows.Add(dr);
            dr = dt.NewRow();
            dr[0] = "套管数量";
            dr[1] = (from DataRow r in stable.Rows
        ...
        private DataTable CreateGuanJieChangDuTongjiTable()
        {
            ...
            row[1] = ((from double l in lengthlist
                      where l >= 0 && l < 2
                      select l).Count() * 1.0 / lengthlist.Count).ToString("0.##");
            row[2] = ((from double l in lengthlist
                      where l >= 2 && l < 5
                      select l).Count() * 1.0 / lengthlist.Count).ToString("0.##");
            row[3] = ((from double l in lengthlist
        ...
        private string CreateGuanjieChangduText(DataTable dt)
        {
            ...
            string text = string.Format( @"管道的内检测数据中{0}的管节数最多，占比达到{1}%；短管节（0≤L＜2）和较短管节（2≤L＜5）的占比分别为{2}%和{3}%，两者占比相对较高，共计{4}%；超长管节（12.3≤L）的比例为{5}%。针对12.3≤L的超长管节，管节数量达到了{6}根，其中最大长度为{7}米，如果按照12m的标准管道长度来计算，该管节可能会漏检{8}条环焊缝。", 
            MaxVcol, maxV.ToString(), duanguanjie.ToString(), Jiaoduanduanguanjie.ToString(), gongji.ToString(),chaochangguanjie.ToString(),
            rowNumV.ToString(),maxLength.ToString("0.##"), rowNumV.ToString() );               
            return text;
        }
        // 超长管径统计表
        private DataTable CreateExceedPipesegmentTable()
        {
            ...
            DataTable dt = new DataTable();
            dt.Columns.Add("上游焊缝编号");
            dt.Columns.Add("X_经度");
            dt.Columns.Add("Y_纬度");
            dt.Columns.Add("Z_85高程(m)");
            dt.Columns.Add("Z_大地高程(m)");
            dt.Columns.Add("里程（m）");
            dt.Columns.Add("钢管长度");
            ...
            List<DataRow> hanfenglist = (from DataRow r in stable.AsEnumerable()
                                         ...
                                         select r).ToList();
            List<double> lengthlist = new List<double>();
            List<int> lengthExceedIndex = new List<int>();
            for (int i = 0; i < hanfenglist.Count - 1; i++)
            {
                lengthlist.Add(Convert.ToDouble(hanfenglist[i + 1][EvConfig.IMUAlignmentMeasureField]) - Convert.ToDouble(hanfenglist[i][EvConfig.IMUAlignmentMeasureField]));
                if (Convert.ToDouble(hanfenglist[i + 1][EvConfig.IMUAlignmentMeasureField]) - Convert.ToDouble(hanfenglist[i][EvConfig.IMUAlignmentMeasureField]) > 12.3)
                {
                    lengthExceedIndex.Add(i);
                }
            }
            foreach (int idx in lengthExceedIndex)
            {
                DataRow imuR = hanfenglist[idx];
                DataRow resultr = dt.NewRow();
                resultr["X_经度"] = imuR["x"];
                resultr["Y_纬度"] = imuR["y"];
                resultr["Z_85高程(m)"] = Convert.ToDouble(imuR["z"]).ToString("0.##");
                resultr["Z_大地高程(m)"] = Convert.ToDouble(imuR["z"]).ToString("0.##");
                resultr["里程（m）"] = Convert.ToDouble(imuR[EvConfig.IMUAlignmentMeasureField]).ToString("0.##");
                resultr["钢管长度"] = lengthlist[idx].ToString("0.##");
                dt.Rows.Add(resultr);
            }         
            return dt;
        }
